...
interface MaterialCategoryStatsCardsProps {
  categories: MaterialCategory[];
}

export function MaterialCategoryStatsCards({ categories }: MaterialCategoryStatsCardsProps) {
  const total = categories.length;
  const ativos = categories.filter((c) => c.active).length;
  const inativos = categories.filter((c) => !c.active).length;
