...
export default function NovaReceberPage() {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      await financialService.createReceivable({
        ...
        issueDate: new Date(),
        status: 'PENDING',
        discount: 0, interest: 0, fine: 0, receivedAmount: 0,
        notes,
      });
      toast.success('Conta a receber criada');
      router.push('/app/financeiro/receber');
    } catch {
  ...
  return (
    <CrudPage title="Nova Conta a Receber" description="Registre um novo recebimento">
      <div className="max-w-lg space-y-4">
        <div className="space-y-2">
          <Label htmlFor="description">Descrição</Label>
          <Input id="description" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Ex: Serviço de instalação" />
        </div>
